function boxes = selective_search_refine(im, bbox)
  ...
  for idx = 1 : size(bbox, 1)
    ...
    box_tmp(:, 1) = box_tmp(:, 1) + repmat(bbox(idx, 1), size(box_tmp, 1), 1);
    box_tmp(:, 3) = box_tmp(:, 3) + repmat(bbox(idx, 1), size(box_tmp, 1), 1);
    box_tmp(:, 2) = box_tmp(:, 2) + repmat(bbox(idx, 2), size(box_tmp, 1), 1);
    box_tmp(:, 4) = box_tmp(:, 4) + repmat(bbox(idx, 2), size(box_tmp, 1), 1);
    % imshow(im);
